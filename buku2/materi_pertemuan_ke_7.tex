...
\subsubsection{INT 33h: Detail dan Koordinat}
Skala koordinat mouse bergantung mode tampilan. Pada mode teks, koordinat biasanya dalam sel karakter; di mode grafik dapat berupa piksel dengan rentang yang dapat diatur (fungsi pembatasan). Selalu periksa nilai kembali inisialisasi driver (\texttt{AX=FFFFh}) sebelum memakai layanan mouse. 
\subsubsection{Debounce Klik dan Drag Sederhana}
Catat transisi status tombol (tekan/lepas) untuk membedakan klik tunggal vs tahan/drag. Simpan posisi terakhir untuk menghitung delta pergerakan dan ambang jarak. 
