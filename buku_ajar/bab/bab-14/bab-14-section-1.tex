...
\begin{enumerate}
  ...
  \item Perbedaan EQU dan variabel (\asm{DW}) yang benar adalah:
  \begin{enumerate}
  ...
  \item Komponen yang diintegrasikan dalam GUI Turbo Assembler (GTASM) meliputi:
  \begin{enumerate}
    \item TASM, TLINK, Turbo Debugger, DOSBox
    \item Hanya TASM
    \item Visual Studio
    \item GCC dan GDB
  \end{enumerate}
  
  \item Format buffer INT 21h AH=0Ah: byte 0 menyimpan:
  \begin{enumerate}
    \item Max length
    \item Actual length (diisi DOS)
...
\begin{enumerate}
  ...
  \item Jelaskan tahapan project development lifecycle untuk assembly programming! Apa yang dianalisis di tahap Requirements dan Design?
  
  \item Jelaskan format buffer untuk INT 21h AH=0Ah (input string)! Apa fungsi byte 0, byte 1, dan byte 2+?
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Jelaskan singkat penerapan tahapan project development (Requirements, Design, Implementation, Testing, Optimization, Documentation) pada proyek yang dikerjakan
\end{enumerate}
