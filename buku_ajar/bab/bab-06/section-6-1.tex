...
\textbf{Contoh dari JMPL.ASM} -- Loop dengan CMP dan JE hingga bertemu karakter \texttt{*}:
\begin{lstlisting}[style=assemblystyle, caption={Loop dengan kondisi (CMP, JE)}]
.DATA
...
Ulang:
    ...
    JE Exit              ; Jika sama, keluar
    MOV DL, Kal[BX]      ; Karakter ke DL
    INT 21h              ; Cetak
    INC BX               ; BX++
    JMP Ulang            ; Ulangi
Exit:
...
\textbf{Contoh dari BRA1.ASM} -- Loop dengan instruksi LOOP:
\begin{lstlisting}[style=assemblystyle, caption={Loop dengan LOOP instruction}]
.DATA
  Kalimat DB 'DIBELAKANG TANDA $ TIDAK BISA DICETAK'
.CODE
    XOR BX, BX           ; BX = 0 (index)
    MOV CX, 25h          ; CX = 37 (counter)
Ulang:
    MOV DL, Kalimat[BX]  ; Ambil karakter
    MOV AH, 02h          ; Servis cetak
    INT 21h              ; Cetak karakter
    INC BX               ; BX++
    LOOP Ulang           ; CX--, jump ke Ulang jika CX != 0
\end{lstlisting}

\textbf{Variasi LOOP:}
\begin{itemize}
  \item \textbf{LOOP}: Loop tanpa kondisi (CX--, jump jika CX $\neq$ 0)
